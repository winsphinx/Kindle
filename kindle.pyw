...
def del_path(path):
    ...
    try:
        ...
        files = [
            os.path.normcase(i) for i in total if os.path.isfile(os.path.join(path, i))
        ]
        dirs = [
            os.path.normcase(i) for i in total if os.path.isdir(os.path.join(path, i))
        ]
        removes = [
            i for i in dirs if i not in [re.sub(r"\.\w+$", ".sdr", j) for j in files]
        ]
        for i in removes:
...
class Kindle(object):
    ...
    def __init__(self):
        ...
        self.root.geometry("%dx%d+10+10" % (w * 0.6, h * 0.8))  # 'wxh+x+y'
